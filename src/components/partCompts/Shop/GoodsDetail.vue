          ...
          <ul class="shop-detail-goods-right-ul">
            <li class="shop-detail-goods-right-li clearfix" v-for="(items, index) in menuList" :key="index">
              ...
              <div class="shop-detail-goods-content" v-for="(item, i) in items.foods" :key="i">
                <!-- <router-link  :to="{ path:'/shop/'+shopId+'/everyFoodDetail', query:{image_path:item.image_path, description: item.description, month_sales: item.month_sales, name: item.name, rating: item.rating, rating_count: item.rating_count, satisfy_rate: item.satisfy_rate, item, shopId}}" tag="div" class="menu_detail_link"> -->
                <router-link  :to="{ path:'/shop/'+shopId+'/everyFoodDetail',query:{shop_id:shopId,category_i:index,current_food:i,foodName:item.name}}">
                  <div class="show-left">
                ...
                <div class="show-right-footer clearfix">
                      ...
                      <div class="add-to-cart-module">
                          <div class="cart-button" v-if="!item.specifications.length">
                            <span class="iconfont icon-jianhao-copy" v-if="foodNum(item)" @click="removeOutCart(item.category_id,item.item_id,item.specfoods[0].food_id)"></span>
                            <span class="goods-number" v-if="foodNum(item)" >{{foodNum(item)}}</span>
                            <span class="iconfont icon-jiahao" @click="addToCart(item.category_id,item.item_id,item.specfoods[0].food_id,item.specfoods[0].name, item.specfoods[0].price,'', item.specfoods[0].packing_fee, item.specfoods[0].sku_id, item.specfoods[0].stock)"></span>
                          </div>
                          <div class="choose-specification" v-else>
                            <span class="iconfont icon-jianhao-copy" v-if="foodNum(item)==1" @click="removeOutCart(item.category_id,item.item_id,item.specfoods[specsIndex].food_id)"></span>
                            <span class="iconfont icon-jianhao-copy" style="color:#ccc" v-if="foodNum(item)>1" @click="showTip"></span>
                            <span class="goods-number" v-if="foodNum(item)" >{{foodNum(item)}}</span>
                            <span class="choose-button" @click="showChooseList(item)">选规格</span>
                          </div>
                        </div>
                    </div>
              </div> 
